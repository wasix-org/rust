...
pub fn futex_wait(futex: &AtomicU32, expected: u32, timeout: Option<Duration>) -> bool {
    let timeout = match timeout {
        Some(timeout) => wasi::OptionTimestamp {
            ...
            u: wasi::OptionTimestampU { some: timeout.as_nanos() as u64 },
        },
...
pub fn futex_wake(futex: &AtomicU32) -> bool {
    unsafe {
        match wasi::futex_wake(futex as *const AtomicU32 as *mut u32) {
...
pub fn futex_wake_all(futex: &AtomicU32) -> bool {
    unsafe {
        match wasi::futex_wake_all(futex as *const AtomicU32 as *mut u32) {
            Ok(wasi::BOOL_TRUE) => true,
            Ok(_) => false,
            Err(_) => false,
        }

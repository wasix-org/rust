...
fn copy_self_contained_objects(
    ...
    if target.needs_crt_begin_end() {
        ...
        if target.contains("-dl") && !builder.config.dry_run() {
            // Delete __main_void.o from libc.a; this is needed because we do --export-all when
            // building DL main modules, and __main_void from libc conflicts with the one
            // generated by rust itself.
            ...
            let host_llvm_bin_dir = command(&host_llvm_config)
                ...
                .trim()
                .to_string();
            let ar_path = PathBuf::from(host_llvm_bin_dir.clone()).join("llvm-ar");
            command(&ar_path)
                .arg("-d")
                .arg(libdir_self_contained.join("libc.a"))
                .arg("__main_void.o")
                .run(builder);
        }

...
use crate::spec::{base, cvs, crt_objects, Cc, LinkerFlavor, Target, LinkSelfContainedDefault};

pub fn target() -> Target {
    let mut options = base::wasm::options();
    options.os = "linux".into();
    options.env = "gnu".into();
    options.families = cvs!["unix"];
    options.vendor = "unknown".into();
